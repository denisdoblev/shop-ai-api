...
export class AiDocumentSourceDto {
  @ApiProperty({ format: 'uuid' }) @IsUUID() chunkId!: string;
  ...
  @ApiProperty({ minimum: 0 }) @IsInt() chunkIndex!: number;
...
export class AiAnswerResponseDto {
  ...
  @ApiProperty({ type: AiDocumentSourceDto, isArray: true })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => AiDocumentSourceDto)
  sources!: AiDocumentSourceDto[];
}
