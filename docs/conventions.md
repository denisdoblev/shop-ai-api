...
- **Explicit:** `RetrievalModule` and `LlmModule` export their services for RAG composition, and `RagModule` exports `RagService` for chat composition. AI workflow services otherwise remain private to their modules, and shared module imports rely on Nest module reuse rather than provider redeclaration.
...
- **Explicit:** LLM generation configuration is validated separately and defaults to Ollama model `qwen3:8b` with a 120000 ms timeout. Grounded RAG defaults to top K 5 and an inclusive cosine similarity threshold of 0.50. LLM connectivity remains lazy, and generation has no automatic retry.
